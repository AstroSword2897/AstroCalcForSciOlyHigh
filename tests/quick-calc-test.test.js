    ...
    test('should calculate escape velocity using Quick Calculate on card', async ({ page }) => {
        ...
        if (rCount > 0 && MCount > 0) {
            ...
            await page.evaluate(() => {
                ...
                if (MInputEl) {
                    MInputEl.value = '5970000000000000000000000'; // Earth mass in kg (no sci notation)
                }
                
                // Dispatch input events after all values are set
                if (rInputEl) rInputEl.dispatchEvent(new Event('input', { bubbles: true }));
                if (MInputEl) MInputEl.dispatchEvent(new Event('input', { bubbles: true }));
            });
            
            // Wait for debounce (500ms) + calculation
