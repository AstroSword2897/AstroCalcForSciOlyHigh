/**
 * IDE Test Runner - Runs calculator tests using Playwright
 * Executes tests until 100% achieved 10 times consecutively
 * 
 * Usage: node run_calculator_tests_ide.js
 */
...
async function runTests() {
    console.log('🧮 CALCULATOR TEST RUNNER - IDE MODE');
    console.log('='.repeat(80));
    console.log('Target: 100% pass rate, 10 times consecutively\n');
    ...
    if (!serverRunning) {
        ...
        console.log('💡 Please start server: python3 -m http.server 8000');
        process.exit(1);
    }
    ...
    try {
        ...
        while (Date.now() - startTime < maxWaitTime) {
            ...
            if (status) {
                if (status.consecutive !== consecutivePasses || status.run !== lastRun) {
                    ...
                    console.log(`Run ${lastRun}: ${status.passed}/${status.total} (${passRate}%) | Consecutive 100%: ${consecutivePasses}/10`);
        ...
        // Keep browser open for 30 seconds to view results
